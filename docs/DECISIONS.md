...
## ADR 008: Expected Net Value Decision Engine & Economic Model Selection
- **Status**: Accepted
- **Context**: In financial recovery operations, classification accuracy or raw log loss does not directly map to profit. Interventions incur real friction costs (e.g. gateway fees, notification spam, human operations). An action must only be taken if its expected gross revenue exceeds its operational friction cost.
- **Decision**:
  - Implement `RecoveryDecisionEngine` optimizing $\arg\max_{a \in \mathcal{A}_{\text{allowed}}} \lfloor \hat{P}(Y(a)=1 \mid X) \cdot \text{amount\_paise} \rfloor - \text{cost\_paise}(a)$.
  - Enforce hard safety guardrails: `NO_ACTION` is always available; `RETRY` is suppressed if `retry_count >= 2`; `ESCALATE` is suppressed for micro-tickets ($< \text{₹}200$).
  - Select model family based strictly on **Validation Economic Decision Quality** ($\Delta \text{Net Revenue}$ vs Baseline & Regret vs Oracle) evaluated against identical potential outcomes under Common Random Numbers.
- **Validation Decision Results (1,500 Cases, ₹4,487,368.00 at risk)**:
  - **Rule Baseline**: ₹2,918,209.00 net recovery (Regret vs Oracle: ₹107,186.00).
  - **Logistic Decision Engine**: **₹3,005,931.00 net recovery (+₹87,722.00 / +3.01% over baseline; Regret vs Oracle: ₹19,464.00)**.
  - **GBM Decision Engine**: ₹2,944,316.00 net recovery (+₹26,107.00 / +0.89% over baseline; Regret vs Oracle: ₹81,079.00).
  - **Oracle**: ₹3,025,395.00 net recovery.
- **Selection**: **Calibrated Logistic Decision Engine** is chosen as the champion model for downstream policy deployment because it captures **81.8% of the Oracle headroom** on the validation set with an average decision margin of ₹373.16.
