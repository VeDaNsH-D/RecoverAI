"""
Live HTTP Integration Smoke Test for RecoverAI LLM Tool-Calling Agent (Milestone 5).
Runs against a live uvicorn server using MockLLMProvider without requiring external API keys.
"""
...
def main():
    print("=" * 75)
    print(" RecoverAI — Milestone 5: LLM Tool-Calling Agent Live HTTP Smoke Test")
    print("=" * 75)
    ...
    try:
        ...
        # ---------------------------------------------------------
        # 1. Execute LLM Tool-Calling Agent Run (POST /api/v1/agent/recover with driver='llm')
        # ---------------------------------------------------------
        print(">>> 1. POST /api/v1/agent/recover (Driver='llm')")
        case_payload = {
            "case_id": "case_smoke_llm_001",
            "customer_id": "cust_smoke_llm_001",
            "amount_paise": 650000,
            "currency": "INR",
            "payment_method": "upi",
        ...
        st, res, headers = make_request(
            ...
            method="POST",
            data=case_payload,
            headers={"X-Request-ID": "req_smoke_llm_init"},
        )
        assert st == 200, f"Expected 200, got {st}: {res}"
        run_id = res["agent_run_id"]
        print(f"Status: {st} | Agent Run ID: {run_id}")
        print(f"Driver: {res['driver_type']} | Decision ID: {res['decision_id']} | Action ID: {res['action_id']}")
        print(f"Recommended: {res['recommended_action']} | Executed: {res['executed_action']}")
        print(f"Operational State: {res['final_operational_state']} | Status: {res['status']}")
        print(f"Telemetry: Total Tokens = {res.get('total_tokens', 0)} | LLM Latency = {res.get('llm_latency_ms', 0.0):.2f}ms")
        ...
        # ---------------------------------------------------------
        # 2. Verify Idempotent Replay with LLM Driver
        # ---------------------------------------------------------
        ...
        for s in steps:
            ...
            tok_info = f" (Tokens: {s.get('llm_prompt_tokens')}+{s.get('llm_completion_tokens')})" if s.get("llm_prompt_tokens") else ""
            print(f"  - Step {s['step_index']}: {s['step_type']}{tool_info}{tok_info} ({s['status']})")
        print()
        ...
        leak_payload["case_id"] = "case_smoke_llm_leak"
        ...
        print("=" * 75)
        print("[+] Live LLM Tool-Calling Recovery Agent HTTP Smoke Test Completed Successfully.")
        print("=" * 75)
